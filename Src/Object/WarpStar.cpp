...
void WarpStar::PlayEffectRotParticle(void)
{
	...
	if (stepEffect_ < 0.0f )
	{
		...
		//エフェクトの大きさ
		SetScalePlayingEffekseer3DEffect(effectRotParticlePlayId_, 5.0f, 5.0f, 5.0f);

		Quaternion rot = transform_.quaRot;
		VECTOR angls = rot.ToEuler();
		SetRotationPlayingEffekseer3DEffect(
			effectRotParticlePlayId_, angls.x, angls.y, angls.z);

		//エフェクトの位置
		VECTOR pos = VAdd(
			transform_.pos, rot.PosAxis(EFFECT_RELATIVE_POS));
		SetPosPlayingEffekseer3DEffect(
			effectRotParticlePlayId_,
			pos.x, pos.y, pos.z);
	}
